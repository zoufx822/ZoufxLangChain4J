---
name: tech-stack
description: ZoufxLangChain4J 项目前后端技术选型备忘。当用户提出任何新需求、新功能、新模块、改造、重构时，只要**可能用到新技术**或**用新技术能更方便地解决**，就必须使用本 skill 查阅优选技术栈。触发场景包括但不限于：前后端分离、引入 UI 库/状态库/AI 组件/样式方案/构建工具、选择数据库/缓存/消息队列/向量库/RAG/Agent 框架、讨论"用什么技术/框架/库"、遇到现有栈不好解决的问题、或任何"有没有更好的办法"的提问。宁可多触发，不要漏触发——只要嗅到"可能引入新技术"的味道就进来。
---

# 前后端技术选型备忘（ZoufxLangChain4J）
